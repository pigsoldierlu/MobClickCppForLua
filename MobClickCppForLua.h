...
public:
    ...
    /** 开启友盟统计,默认以BATCH方式发送log.
     ...
     @param channelId 渠道名称,为0或""时,ios默认会被被当作@"App Store"渠道，android默认为“Unknown”。
     @return void
     */
    ...
    /** 自定义事件,数量统计.
     ...
     @param  label 分类标签。不同的标签会分别进行统计，方便同一事件的不同标签的对比,为0或空字符串时后台会生成和eventId同名的标签.
     @return void.
     */
    ...
    /** 通过关卡.
     @param level 关卡,如果level == 0 则为当前关卡
     @return void
     */
